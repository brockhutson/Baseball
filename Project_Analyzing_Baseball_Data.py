# -*- coding: utf-8 -*-
"""
Created on Mon Sep  9 10:48:25 2019
...
def filter_by_year(statistics, year, yearid):
    ...
    req_year = [row for row in statistics if row[yearid] == str(year)]
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for item in statistics: 
        player = (item[info['playerid']], formula(info, item)) #2-tuple of ('playerID', formula result)
        result_list.append(player)
 
    sorted_result_list = sorted(result_list, key=lambda comp_form: comp_form[1], reverse=True)
        
    return sorted_result_list[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    Inputs:
      ...
      top_ids_and_stats - list of tuples containing player IDs and
                          computed statistics (asdfsadf01, 0.29651561564)
    Outputs:
    ...
    mastercsv = read_csv_as_nested_dict(info['masterfile'], info['playerid'], info['separator'], info['quote']) #reads in Master_2016.csv as nested_dict

    string_list = []
    
    for tuple in top_ids_and_stats:
        playername = str(mastercsv[tuple[0]][info['firstname']]) + ' ' + str(mastercsv[tuple[0]][info['lastname']])
        stat = ("%.3f" % tuple[1])
        data = stat + ' --- ' + playername
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    batstats = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    
    stats_by_year = filter_by_year(batstats, year, info['yearid']) #Returns list of batting statistics dicts for req year

    best_of_year = top_player_ids(info, stats_by_year, formula, numplayers )
    
    best_players = lookup_player_names(info, best_of_year)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for dic in statistics:
        results.setdefault(dic[playerid], {playerid : dic[playerid]})
        for k,v in dic.items():
            if k in fields:
                results[dic[playerid]][k] = results[dic[playerid]].get(k,0) + int(v)
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    batstats = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote']) # list dict
    
    agg_stats = aggregate_by_player_id(batstats, info['playerid'], info['battingfields']) # returns nested dict

    
    career_stats = []
    for keys, values in agg_stats.items():
        career_stats.append(values)
    
    top_players = top_player_ids(info, career_stats, formula, numplayers)
    
    players_by_name = lookup_player_names(info, top_players)
...
#    print('Top batting averages by year')
#    year_range= [year for year in range(1923, 2016)] 
#    for year in year_range:
#        stat = compute_top_stats_year(baseballdatainfo, batting_average, 1, year)
#        for player in stat:
#            print(str(year)+':', player)


    print("Top 10 batting averages in 1981")
    top_batting_average_1981 = compute_top_stats_year(baseballdatainfo, batting_average, 10, 1981)
    for player in top_batting_average_1981:
        print(player)
    print("")
